"""Aggregate IR (retrieval) metric helpers.

Named ``ir_aggregate`` to disambiguate from ``evaluation/aggregate.py`` (the
report builder). This module only batch-computes retrieval metrics.
"""
...
def compute_ir_metrics(
    ...
    for kk in k_values:
        results[f"Recall@{kk}"] = (
            ...
            if len(recall_at_ks[kk]) > 0
            else 0.0
        )
        results[f"NDCG@{kk}"] = (
            sum(ndcg_at_ks[kk]) / len(ndcg_at_ks[kk])
            if len(ndcg_at_ks[kk]) > 0
            else 0.0
        )
...
def log_ir_metrics(
    results: Dict[str, float], logger, k_values: Optional[List[int]] = None
):
