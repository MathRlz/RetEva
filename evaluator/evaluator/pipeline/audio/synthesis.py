...
class AudioSynthesizer:
    ...
    def __init__(self, config):
        ...
        """
        self.config = config
        self.skip_cache = getattr(config, "skip_cache", False)
        self._cache_hits = 0
    ...
    def synthesize(self, text: str, output_path: Optional[str] = None) -> np.ndarray:
        ...
        # Early-exit: output file already exists — load and return without running TTS
        if not self.skip_cache and output_path and os.path.exists(output_path):
            audio = self._load_audio(output_path)
            if audio is not None:
                self._cache_hits += 1
                logger.info("TTS cache hit (file): %s", output_path)
                return audio

        # Check text-hash cache
        if self.cache_dir:
            cache_key = self._get_cache_key(text)
            cached_audio = self._load_from_cache(cache_key)
            if cached_audio is not None:
                self._cache_hits += 1
                logger.info("TTS cache hit (hash %s...)", cache_key[:16])
