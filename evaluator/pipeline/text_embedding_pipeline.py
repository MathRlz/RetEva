...
class TextEmbeddingPipeline(CacheMixin):
    ...
    def process_batch(self, texts: List[str], show_progress: bool = False) -> np.ndarray:
        """
        ...
        if uncached_texts:
            with TimingContext(f"Text embedding batch ({len(uncached_texts)} texts)", logger):
                batch_embeddings = self.model.encode(uncached_texts, show_progress=show_progress)
