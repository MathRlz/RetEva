...
class RunState:
    ...
    # id → node kind of the EXECUTED graph. Handlers answer per-node questions ("was my
    # query_text produced by an ASR node?") from bindings + this map — graph truth —
    # instead of run-global mode labels / pipeline presence, which misfire in mixed graphs.
    node_kinds: Dict[str, str] = field(default_factory=dict, metadata=_SHARED)
