...
def evaluate_phased(
    ...
    if mode in ["asr_text_retrieval", "audio_emb_retrieval", "audio_text_retrieval"]:
        logger.info("=" * 50)
        ...
        if len(all_retrieved) > 0 and len(all_ground_truth) > 0 and can_debug_with_search:
            ...
            # Re-run first 3 searches to get scores
            ...
            for i in range(min(3, len(all_embeddings))):
                ...
                if i < len(all_relevance) and len(all_relevance[i]) > 0:
                    gt_text = next(iter(all_relevance[i].keys()))
                else:
                    gt_text = str(all_ground_truth[i])
                
                logger.info(f"\nQuery {i+1}:")
                logger.info(f"  Ground truth: '{gt_text[:80]}'")
                logger.info(f"  Query embedding norm: {np.linalg.norm(emb_array[i]):.4f}")
                logger.info(f"  Top 5 retrieved (with scores):")
                
                for j, (text, score) in enumerate(results_with_scores[:5], 1):
                    match = "✓" if text == gt_text else "✗"
                    logger.info(f"    {j}. [{match}] Score: {score:.4f} | Text: '{text[:80]}'")
                
                # Check if ground truth is anywhere in top 10
                gt_rank = None
                for rank, (text, score) in enumerate(results_with_scores, 1):
                    if text == gt_text:
                        gt_rank = rank
                        logger.info(f"  Ground truth found at rank: {gt_rank} (score: {score:.4f})")
                        break
                
                if gt_rank is None:
                    logger.info(f"  Ground truth NOT found in top-{k}")
                    # Check if it exists in DB at all
                    payload_keys = [_payload_to_key(p) for p in getattr(retrieval_pipeline.vector_store, 'payloads', [])]
                    if str(gt_text) in payload_keys:
                        logger.info(f"  (Ground truth EXISTS in DB)")
                    else:
                        logger.info(f"  (Ground truth NOT in DB!)")
    ...
    if mode in ["asr_text_retrieval", "asr_only"]:
        ...
        wer_scores = []
        cer_scores = []
        for gt_text, hyp_text in zip(all_ground_truth, all_hypotheses):
    ...
    if mode != "asr_only":
        ...
        if judge_config is not None and getattr(judge_config, "enabled", False):
            ...
            logger.info("Running LLM-as-judge over query traces...")
            judge_results = run_llm_judging(
                results["query_traces"],
                api_base=judge_config.get_api_base(),  # Support local server
                model=judge_config.model,
                api_key_env=judge_config.api_key_env,
                temperature=judge_config.temperature,
                max_cases=judge_config.max_cases,
