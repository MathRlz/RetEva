"""The flat metrics mapping returned by ``run_graph``.

``RunResults`` is a type alias for a plain dict; consumers read well-known keys.

Key groups (all optional — present only for the relevant mode / enabled feature):
    meta        pipeline_mode, phased, oracle_mode, latency
    ASR         asr, WER, CER (+ opt-in corrected_*/ceer_rx, §7)
